import numpy as np
# import matplotlib.pyplot as plt
import scipy.linalg as linalg
from math import sqrt
import sys

from sklearn.cluster import kmeans_plusplus
import mlgrad.inventory as inventory
# from mlgrad.af import averaging_function
from mlgrad.avragg import ArithMean
from mlgrad.funcs import Square

det = linalg.det
einsum = np.einsum
einsum_path = np.einsum_path
nonzero = np.nonzero
arange = np.arange
empty = np.empty
average = np.average

def norm2(x):
    return (x @ x)

def mnorm2(x, S1):
    return (S1 @ x) @ x

class KMeansBase:
    #
    def predict(self, X):
        Y = np.empty(len(X), 'i')
        Is = self.find_clusters(X)
        for j in range(self.q):
            Ij = Is[j]
            Y[Ij] = j
        return Y
    #
    def initial_locations(self, X):
        return kmeans_plusplus(X, self.q)[0]
    #
    def _eval_dists(self, X):
        """Evaluate array (N x q) with square distances from the points to the cluster centers.
        """
        c = self.c
        DD = np.empty((self.q, len(X)), "d")
        path, _ = einsum_path("ni,ni->n", X, X, optimize='optimal')
        for j in range(self.q):
            Xj = X - c[j]
            einsum("ni,ni->n", Xj, Xj, optimize=path, out=DD[j])
        return DD
    #
    def eval_dists(self, X):
        DD = self._eval_dists(X)
        ds = DD.min(axis=0)
        return ds
    #
    def eval_qval(self, X):
        DD = self._eval_dists(X)
        return sqrt(D.mean())
    #
    def find_clusters(self, X):
        DD = self._eval_dists(X)

        D = DD.min(axis=0)
        II = arange(len(X))
        Is = []
        for j in range(self.q):
            Ij = II[DD[j] == D]
            # if Ij.size == 0:
            #     raise TypeError("empty cluster")
            Is.append(Ij)
        return Is
    #

class KMeans(KMeansBase):
    #
    def __init__(self, q, tol=1.0e-8, n_iter=1000, verbose=False):
        self.q = q
        self.n_iter = n_iter
        self.tol = tol
        self.verbose = verbose
        self.S1 = None
    #
    def find_locations(self, X, Is):
        mean = np.mean
        c = np.empty((self.q, X.shape[1]), 'd')
        for j in range(self.q):
            Ij = Is[j]
            Xj = X[Ij]
            c[j,:] = Xj.mean(axis=0)
        return c
    #
    def stop_condition(self, c, c_prev):
        dc = np.abs(c - c_prev)
        dc2 = dc * dc
        dmax2 = max(dc2.sum(axis=1))
        dmax = sqrt(dmax2)
        # print(dmax)
        self.qvals.append(dmax)
        if dmax < self.tol:
            return True

        return False
    #
    def fit(self, X):
        self.c = self.initial_locations(X)
        self.qvals = []
        self.qvals = []
        for K in range(self.n_iter):
            c_prev = self.c.copy()
            self.Is = self.find_clusters(X)
            self.c = self.find_locations(X, self.Is)
            if self.stop_condition(self.c, c_prev):
                break
        self.K = K + 1

class RKMeans(KMeansBase):
    #
    def __init__(self, q, avrfunc=None, tol=1.0e-8, n_iter_c=100, n_iter=500, verbose=False):
        self.q = q
        self.n_iter = n_iter
        self.n_iter_c = n_iter_c
        self.tol = tol
        if avrfunc is None:
            self.avrfunc = aggfuncs.ArithMean()
        else:
            self.avrfunc = avrfunc
        self.verbose = verbose
        self.S1 = None
    #
    def find_locations(self, X, Is, G):
        n = X.shape[1]
        c = np.zeros((self.q, n), 'd')
        for j in range(self.q):
            Ij = Is[j]
            Gj = G[Ij]
            c[j,:] = X[Ij].T @ Gj / Gj.sum()
        return c
    #
    def stop_condition(self, qval, qval_prev):
        if abs(qval - qval_prev) / (1 + self.qval_min) < self.tol:
            return True

        # if len(self.qval_mins) > 1 and \
        #     abs(qval - qval_prev) / (1 + self.qval_min) < self.tol

        return False
    #
    def eval_qval(self, X):
        ds = self.eval_dists(X)
        dd = self.avrfunc.evaluate(ds)
        return np.sqrt(dd)
    #
    def fit_locations(self, X):
        N = X.shape[0]
        self.c_min = self.c.copy()

        self.ds = self.eval_dists(X)
        dd = self.avrfunc.evaluate(self.ds)
        qval = self.qval_min = np.sqrt(dd)
        self.qval_min_prev = self.qval_min
        for K in range(self.n_iter_c):
            qval_prev = qval
            self.avrfunc.evaluate(self.ds)
            G = self.avrfunc.gradient(self.ds)
            self.Is = self.find_clusters(X)
            self.c = self.find_locations(X, self.Is, G)

            self.ds = self.eval_dists(X)
            dd = self.avrfunc.evaluate(self.ds)
            qval = np.sqrt(dd)
            self.qvals.append(qval)

            if qval <= self.qval_min:
                self.qval_min_prev = self.qval_min
                self.qval_min = qval
                self.c_min = self.c.copy()

            if self.stop_condition(qval, qval_prev):
                break
            # if self.stop_condition(self.qval_min_prev, self.qval_min):
            #     break

        self.K = K + 1
        self.c = self.c_min
    #
    def fit(self, X):
        q = self.q
        n = X.shape[1]
        N = X.shape[0]
        self.c = self.c_min = self.initial_locations(X)
        self.qvals = []
        # self.qval_mins = []
        self.qvals2 = []
        qval2 = self.qval_min = self.eval_qval(X)

        for K in range(self.n_iter):
            qval_prev2 = qval2
            self.fit_locations(X)
            qval2 = self.eval_qval(X)
            self.qvals2.append(qval2)
            if self.stop_condition(qval2, qval_prev2):
                break
        self.K = K + 1


class KMeansMahalanobisBase(KMeansBase):
    #
    def _eval_dists(self, X):
        if self.DD is None or self.DD.shape != (self.q, len(X)):
            self.DD = np.zeros((self.q, len(X)), "d")
        # path, _ = einsum_path("ni,ij,nj->n", X, S1[0], X, optimize='optimal')
        for j in range(self.q):
            inventory.mahalanobis_distance(X, self.S[j], self.c[j], Y=self.DD[j])
            self.DD[j] *= self.scales[j]
            # Xj = X - c[j]
            # einsum("ni,ij,nj->n", Xj, S1[j], Xj, optimize=path, out=DD[j])
        return self.DD
    #
    def set_weights(self, weights):
        self.weights = np.asarray(weights)
    #
    def eval_dists(self, X):
        DD = self._eval_dists(X)
        ds = DD.min(axis=0)
        return ds
    #
    def eval_qval(self, X):
        DD = self._eval_dists(X)
        D = DD.min(axis=0)
        return sqrt(D.mean())
    #
    def eval_qval2(self, X):
        DD = self._eval_dists(X)
        D = DD.min(axis=0)
        return D.mean()
    #

class KMeansMahalanobis(KMeansMahalanobisBase):
    #
    def __init__(self, q, rho_func=None,
                 tol=1.0e-8, n_iter_c=100, n_iter_s=22, n_iter=500, verbose=False):
        self.q = q
        if rho_func is None:
            self.rho_func = funcs.Square()
        else:
            self.rho_func = rho_func
        self.n_iter = n_iter
        self.n_iter_c = n_iter_c
        self.n_iter_s = n_iter_s
        self.tol = tol
        self.qvals = []
        self.weights = None
        self.verbose = verbose
        self.DD = None
    #
    def find_locations(self, X, Is):
        n = X.shape[1]
        c = np.zeros((self.q, n))
        weights = self.weights
        for j in range(self.q):
            Ij = Is[j]
            if Ij.size == 0:
                continue
            if weights is not None:
                c[j,:] = np.average(X[Ij], axis=0, weights=weights[Ij])
            else:
                c[j,:] = X[Ij].mean(axis=0)
        self.c = c
    #
    def find_scales(self, X, Is):
        n = X.shape[1]
        Is = self.find_clusters(X)
        scales = self.scales.copy()
        weights = self.weights
        for j in range(self.q):
            Ij = Is[j]
            if Ij.size == 0:
                continue
            dj = inventory.mahalanobis_distance(X[Ij], self.S[j], self.c[j])
            if weights is not None:
                scales[j] = np.average(dj, weights=weights[Ij])
            else:
                scales[j] = dj.mean()
        scales = n / scales
        self.scales = self.q * scales / scales.sum()
    #
    def find_scatters(self, X, Is):
        solve = linalg.solve
        det = linalg.det

        n = X.shape[1]
        E = np.eye(n)

        S = []
        weights = self.weights
        c = self.c
        for j in range(self.q):
            Ij = Is[j]
            if Ij.size == 0:
                S.append(self.S[j])
                continue
            if weights is not None:
                Sj = inventory.covariance_matrix_weighted(X[Ij], weights[Ij], c[j])
            else:
                Sj = inventory.covariance_matrix(X[Ij], c[j])
            Sj1 = solve(Sj, E)
            Sj1 = (Sj1.T + Sj1) / 2
            Sj1 /= det(Sj1) ** (1.0/n)
            S.append(np.ascontiguousarray(Sj1))

        self.S = S
    #
    def fit_locations(self, X):
        # qval_min = qval_prevmin = qval = self.eval_qval(X)
        qval = self.eval_qval(X)
        stop_cond = inventory.StopCondition(qval, tol=self.tol)
        for K in range(self.n_iter_c):
            Is = self.find_clusters(X)
            self.find_locations(X, Is)

            qval = self.eval_qval(X)
            self.qvals.append(qval)
            if stop_cond.is_minval(qval):
                self.c_min = self.c
            # if qval <= qval_min:
            #     qval_prevmin = qval_min
            #     qval_min = qval
            #     c_min = c

            if stop_cond.stop_condition():
                break
            # if abs(qval_min - qval_prevmin)  < self.tol * (1 + abs(qval_min)):
            #     break

        self.c = self.c_min
        # self.Is = Is
        self.K_c = K+1
    #
    def fit_scales(self, X):
        # qval_min = qval_prevmin = qval = self.eval_qval(X)
        qval = self.eval_qval(X)
        stop_cond = inventory.StopCondition(qval, tol=self.tol)
        for K in range(self.n_iter_c):
            Is = self.find_clusters(X)
            self.find_scales(X, Is)

            qval = self.eval_qval(X)
            self.qvals.append(qval)
            if stop_cond.is_minval(qval):
                self.c_min = self.c
            # if qval <= qval_min:
            #     qval_prevmin = qval_min
            #     qval_min = qval
            #     c_min = c

            if stop_cond.stop_condition():
                break
            # if abs(qval_min - qval_prevmin)  < self.tol * (1 + abs(qval_min)):
            #     break

        # self.c = self.c_min
        # self.Is = Is
        self.K_c = K+1
    #
    def fit_scatters(self, X):
        qval_min = qval_prevmin = qval = self.eval_qval(X)
        S_min = self.S
        for K in range(self.n_iter_s):
            Is = self.find_clusters(X)
            self.find_scatters(X, Is)

            qval = self.eval_qval(X)
            self.qvals.append(qval)
            if qval <= qval_min:
                qval_prevmin = qval_min
                qval_min = qval
                S_min = self.S

            if abs(qval_min - qval_prevmin)  < self.tol * (1 + abs(qval_min)):
                break

        self.S = S_min
        self.K_S = K+1
    #
    def fit(self, X):
        n = X.shape[1]
        self.c = self.c_min = self.initial_locations(X)
        self.S = self.S_min = [np.identity(n) for j in range(self.q)]
        self.scales = np.ones(n)
        self.qvals2 = []
        qval2 = qval2_min = qval2_prevmin = self.eval_qval(X)

        for K in range(self.n_iter):
            self.fit_locations(X)
            self.fit_scatters(X)
            self.fit_scales(X)

            qval2 = self.eval_qval(X)
            self.qvals2.append(qval2)
            if qval2 <= qval2_min:
                qval2_prevmin = qval2_min
                qval2_min = qval2

            if abs(qval2_min - qval2_prevmin)  < self.tol * (1 + abs(qval2_min)):
                break

        self.K = K + 1

class RKMeansMahalanobis(KMeansMahalanobis):
    #
    def __init__(self, q, avrfunc=None, rho_func=None, tol=1.0e-6,
                 n_iter_c=22, n_iter_s=11, n_iter=100, verbose=False):
        self.q = q
        self.n_iter = n_iter
        self.n_iter_c = n_iter_c
        self.n_iter_s = n_iter_s
        self.tol = tol
        if avrfunc is None:
            self.avrfunc = ArithMean()
        else:
            self.avrfunc = avrfunc
        if rho_func is None:
            self.rho_func = Square()
        else:
            self.rho_func = rho_func
        self.verbose = verbose
        self.weights = None
        self.DD = None
        self.c = None
        self.S = None
    #
    def eval_qval(self, X):
        ds = self.eval_dists(X)
        dd = self.avrfunc.evaluate(ds)
        return np.sqrt(dd)
    #
    def eval_qval2(self, X):
        ds = self.eval_dists(X)
        dd = self.avrfunc.evaluate(ds)
        return dd
    #
    def fit_locations(self, X, n_iter=None):
        ds = self.eval_dists(X)
        qval = np.sqrt(self.avrfunc.evaluate(ds))
        stop_cond = inventory.StopCondition(qval, tol=self.tol)
        # qval_min_prev = qval_min * 2

        weights = self.weights

        if n_iter is None:
            n_iter = self.n_iter_c
        for K in range(n_iter):
            Is = self.find_clusters(X)
            self.find_locations(X, Is)

            ds = self.eval_dists(X)
            qval = np.sqrt(self.avrfunc.evaluate(ds))
            self.weights = self.avrfunc.weights(ds)
            self.qvals.append(qval)

            if stop_cond.is_minval(qval):
                self.c_min = self.c.copy()

            # if qval < qval_min:
            #     qval_min_prev = qval_min
            #     qval_min = qval
            #     self.c_min = self.c
            # elif qval < qval_min_prev:
            #     qval_min_prev = qval

            if stop_cond.stop_condition():
                break
            # if abs(qval_min_prev - qval_min) < self.tol * (1 + abs(qval_min)):
            #     break

        self.K = K + 1
        self.c = self.c_min
    #
    def fit_scatters(self, X, n_iter=None):
        ds = self.eval_dists(X)
        qval = np.sqrt(self.avrfunc.evaluate(ds))
        stop_cond = inventory.StopCondition(qval, tol=self.tol)
        # qval_min_prev = qval_min * 2

        if n_iter is None:
            n_iter = self.n_iter_s
        for K in range(n_iter):
            Is = self.find_clusters(X)
            self.find_scatters(X, Is)

            ds = self.eval_dists(X)
            qval = np.sqrt(self.avrfunc.evaluate(ds))
            self.weights = self.avrfunc.weights(ds)
            self.qvals.append(qval)

            if stop_cond.is_minval(qval):
                self.S_min = [Sj.copy() for Sj in self.S]
            # if qval < qval_min:
            #     qval_min_prev = qval_min
            #     qval_min = qval
            #     self.S_min = self.S
            # elif qval < qval_min_prev:
            #     qval_min_prev = qval

            if stop_cond.stop_condition():
                break
            # if abs(qval_min_prev - qval_min) < self.tol * (1 + abs(qval_min)):
            #     break

        self.K = K + 1
        self.S = self.S_min
    #
    def fit_scales(self, X, n_iter=None):
        ds = self.eval_dists(X)
        qval = np.sqrt(self.avrfunc.evaluate(ds))
        stop_cond = inventory.StopCondition(qval, tol=self.tol)
        # qval_min_prev = qval_min * 2

        weights = self.weights

        if n_iter is None:
            n_iter = self.n_iter_s
        for K in range(n_iter):
            Is = self.find_clusters(X)
            self.find_scales(X, Is)

            ds = self.eval_dists(X)
            qval = np.sqrt(self.avrfunc.evaluate(ds))
            self.weights = self.avrfunc.weights(ds)
            self.qvals.append(qval)

            if stop_cond.is_minval(qval):
                self.scales_min = self.scales.copy()

            # if qval < qval_min:
            #     qval_min_prev = qval_min
            #     qval_min = qval
            #     self.c_min = self.c
            # elif qval < qval_min_prev:
            #     qval_min_prev = qval

            if stop_cond.stop_condition():
                break
            # if abs(qval_min_prev - qval_min) < self.tol * (1 + abs(qval_min)):
            #     break

        self.K = K + 1
        self.scales = self.scales_min
    #
    def fit(self, X, warm=False, method=None):
        q = self.q
        n = X.shape[1]
        N = X.shape[0]
        if not warm or self.c is None or self.S is None:
            self.c = self.c_min = self.initial_locations(X)
            self.S = self.S_min = [np.identity(n) for j in range(q)]
            self.scales = self.scales_min = np.ones(self.q)
        self.qvals = []
        self.qvals2 = []

        ds = self.eval_dists(X)
        qval = np.sqrt(self.avrfunc.evaluate(ds))

        self.weights = self.avrfunc.weights(ds)

        # qval = self.eval_qval(X)
        stop_cond = inventory.StopCondition(qval, tol=self.tol)
        # qval_min_prev = qval_min * 2

        if method == "simple":
            n_iter_c = n_iter_s = 1
        else:
            n_iter_c = n_iter_s = None

        for K in range(self.n_iter):
            self.fit_locations(X, n_iter_c)
            self.fit_scatters(X, n_iter_s)
            self.fit_scales(X, n_iter_s)
            print(self.scales)

            ds = self.eval_dists(X)
            qval = np.sqrt(self.avrfunc.evaluate(ds))
            self.qvals2.append(qval)

            self.weights = self.avrfunc.weights(ds)

            stop_cond.is_minval(qval)
            # if qval < qval_min:
            #     qval_min_prev = qval_min
            #     qval_min = qval
            # elif qval < qval_min_prev:
            #     qval_min_prev = qval

            if stop_cond.stop_condition():
                break
            # if abs(qval_min_prev - qval_min) < self.tol * (1 + abs(qval_min)):
            #     break

        self.K = K + 1
        self.c = self.c_min
        self.S = self.S_min
        self.scales = self.scales_min

