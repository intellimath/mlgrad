...
from mlgrad.averager import _get_averager
import numpy as np
...
def erisk(X, Y, mod, loss_func, regnorm=None, weights=None, tau=0.001, batch=None, avg=None):
    if avg is not None:
        er = MRisk(X, Y, mod, loss_func, avg=avg, regnorm=regnorm, tau=tau, batch=batch)
